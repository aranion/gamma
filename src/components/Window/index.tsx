...
export function Window(props: Props) {
  const { windowItem, dragElement, desktopList, desktopId, setDesktopList, refDesktops } = props
  ...
  function clientWidth(): number {
    return refDesktops.current.clientWidth === 0 ? document.body.clientWidth : refDesktops.current.clientWidth
  }

  function clientHeight(): number {
    return refDesktops.current.clientHeight === 0 ? document.body.clientHeight : refDesktops.current.clientHeight
  }
  ...
  function moveWindowDesktop(): void {
    desktopList.map((el) => {
      if (el.desktopId === desktopId) {
        let del = el.windows.findIndex((item) => item.windowId === windowItem.windowId)
  ...
  return (
    <div className={classes.window} ref={windowItem.ref}>
      <div ref={windowItem.refMoveElem} className={classes['window__header']} onChange={(e) => dragElement(e, windowItem.refMoveElem, windowItem)}>
