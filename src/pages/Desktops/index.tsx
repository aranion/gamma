...
export function Desktops() {
  ...
  function dragElement(elmnt: HTMLDivElement, refMoveElem: HTMLDivElement, windowItem: WindowItem) {
    if (refMoveElem) {
    ...
    function elementDrag(e: MouseEvent) {
      ...
      windowItem.position.pos3 = e.clientX
      windowItem.position.pos4 = e.clientY
      windowItem.top = checkMaxBorder(elmnt.offsetTop - windowItem.position.pos2, Direction.vertically)
      windowItem.left = checkMaxBorder(elmnt.offsetLeft - windowItem.position.pos1, Direction.horizontally)
      ...
      desktopList.forEach((el) => {
        el.windows.forEach((item) => {
          if (item.windowId === selectWindowId) {
            selectWIndows.splice(selectWIndows.length, 1, selectWIndows.splice(selectWIndows.indexOf(selectWindowId), 1)[0])

            item.zIndex = selectWIndows.indexOf(item.windowId)
          }
          if (selectWIndows.indexOf(item.windowId) !== -1) item.zIndex = selectWIndows.indexOf(item.windowId)
        })
      })
  ...
  return (
    <div className={`${classes.desktops} ${classes['no-select']}`} ref={refDesktops}>
      {desktopList.map((el) => (
        <Desktop
          key={el.desktopId}
